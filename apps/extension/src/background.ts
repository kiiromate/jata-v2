...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.action === 'CAPTURE_SELECTION' && sender.tab) {
    const details = message.data as { textContent: string; sourceUrl: string; pageTitle: string };
    captureJobToInbox(
      { jobTitle: details.pageTitle, companyName: '', jobUrl: details.sourceUrl, jobDescription: details.textContent },
      { captureSurface: (message.captureSurface as string) || 'selection_popover' },
    )
      .then((result) => sendResponse(result))
      .catch((err: unknown) => {
        const msg = err instanceof Error ? err.message : 'Capture failed.';
        buildJataWebUrl('/capture-inbox').then((openUrl) =>
          sendResponse({ state: 'error', message: msg, openUrl }),
        );
      });
    return true;
  }
  ...
  if (message.action === 'APPEND_SELECTION' && sender.tab) {
    ...
    const textContent = (message.data as { textContent: string }).textContent;
    chrome.storage.local.get(POPUP_DRAFT_KEY, (items) => {
      const draft = items[POPUP_DRAFT_KEY] as { jobDescription?: string } | undefined;
      const updated = {
        ...(draft || {}),
  ...
  if (message.action === 'elementSelected' && sender.tab) {
    chrome.storage.session.get(['jata-pick-pending', 'pickedFields'], (items) => {
      const pending = items['jata-pick-pending'] as
        | { field: string; startedAt: number }
        | undefined;
      if (pending && Date.now() - pending.startedAt < 2 * 60 * 1000) {
        const fields = items.pickedFields || {};
        fields[pending.field] = (message.data as { textContent?: string })?.textContent ?? '';
