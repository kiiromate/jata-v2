...
interface CaptureInboxResponse {
  id?: string;
  duplicateStatus?: 'unknown' | 'unique' | 'possible_duplicate' | 'duplicate';
  title?: string;
  company?: string;
}
...
export function buildCaptureInboxBody(details: JobDetails, metadata: Record<string, string | undefined> = {}) {
...
export async function captureJobToInbox(
  details: JobDetails,
  ...
  const payload = await response.json().catch(() => ({})) as CaptureInboxResponse & {
    error?: string;
    details?: unknown;
  };
