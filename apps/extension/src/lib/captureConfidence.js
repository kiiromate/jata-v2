...
function resolveAdapterId(url, source) {
    if (source) {
    ...
    if (/\/jobs?\/|\/careers?\/|\/positions?\//i.test(url))
        return 'generic_job_page';
    return 'generic_opportunity';
}
export function computeCaptureConfidence(input) {
    ...
    const confidenceLabel = score >= 0.80 ? 'strong' : score >= 0.55 ? 'review_recommended' : 'weak';
    return {
