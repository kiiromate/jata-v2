...
function showPopover(rect: DOMRect, selectedText: string) {
  cleanupPopover();
  ...
  const onScroll = () => {
    if (Math.abs(window.scrollY - scrollY) > 100) cleanupPopover();
  };
  const onKeydown = (e: KeyboardEvent) => {
    if (e.key === 'Escape') cleanupPopover();
  };
  const onMousedown = (e: MouseEvent) => {
    if (!pop.contains(e.target as Node)) cleanupPopover();
  };
...
document.addEventListener('selectionchange', () => {
  ...
  debounceTimer = setTimeout(() => {
    ...
    if (range.startContainer.ownerDocument !== document) return;
