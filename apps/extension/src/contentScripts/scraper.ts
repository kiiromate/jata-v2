...
interface CapturePageResponse {
  state?: 'captured' | 'duplicate' | 'possible_duplicate' | 'queued' | 'error';
  message?: string;
  openUrl?: string;
}
...
const textFromSelectors = (selectors: string[], maxLength?: number): string => {
...
const cleanTitle = (value?: string | null): string => {
  ...
  const likelyTitle = title.match(
    /\b([A-Z][\w+/#.,&() -]{2,120}?\b(?:Engineer|Developer|Designer|Manager|Lead|Director|Analyst|Specialist|Coordinator|Consultant|Associate|Intern|Officer|Architect|Scientist|Product|Operations|Marketing|Sales|Finance|Data)\b[\w+/#.,&() -]*)/i,
  );

  return cleanText(likelyTitle?.[1] || title, 140);
};

const cleanJobDescription = (value?: string | null): string => {
...
const companyFromUrl = (url: string): string => {
  try {
...
const findJobPostingJson = (): Record<string, unknown> | null => {
...
const companyFromJobPosting = (posting: Record<string, unknown> | null): string => {
  const hiringOrganization = posting?.hiringOrganization;
  if (!hiringOrganization || typeof hiringOrganization !== 'object') return '';
...
const getSourceFromUrl = (url: string): string => {
  ...
  if (/ashbyhq\.com|jobs\.ashby\.io/i.test(url)) return 'Ashby';
 ...
 */
const autoExtractJobDetails = (): ExtractedJobDetails => {
...
function isLikelyOpportunityPage(): boolean {
  if (isJataAppPage()) return false;

  const url = window.location.href;
  const text = document.body?.innerText?.slice(0, 12000).toLowerCase() || '';
  const urlLooksLikeJob =
    /linkedin\.com\/jobs|indeed\.com|greenhouse\.io|boards\.greenhouse\.io|lever\.co|myworkdayjobs\.com|workable\.com|ashbyhq\.com|smartrecruiters\.com|wellfound\.com|ziprecruiter\.com|\/jobs?\/|\/careers?\/|\/positions?\//i.test(url);
  const textLooksLikeJob =
    /\b(apply now|job description|responsibilities|requirements|qualifications|about the role|about this role|employment type)\b/.test(text);
...
function isJataAppPage(): boolean {
...
function setPillState(pill: HTMLButtonElement, state: CapturePageResponse['state'], message?: string) {
...
function injectCapturePill() {
  if (!isLikelyOpportunityPage() || document.getElementById('jata-capture-pill')) return;
